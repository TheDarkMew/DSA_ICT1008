...
def findRouteGraph(start,end):
    ...
    for i in range(0, len(dj.hdbbus)):
        #all address will be shown
        net.add_node(dj.hdbbus[i][0], size=5)
        net.add_node(dj.hdbbus[i][1], size=5)
        ...
        net.add_edge(dj.hdbbus[i][0], dj.hdbbus[i][1], weight=dj.hdbbus[i][2])
    #print(net.edges)
